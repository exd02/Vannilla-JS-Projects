...
function copyToClip() {
  ...
  setTimeout(backToNormalCopyBtn, 1700)
}

function backToNormalCopyBtn() {
...
form.addEventListener("submit", function (e) {
  // prevent the submition to the server
  e.preventDefault()

  let numParagraphs
  if (!amount.value) // check for invalid form
    numParagraphs = 1
  else
    numParagraphs = parseInt(amount.value)

  let txt = ''
  for (let i=0; i<numParagraphs; i++) {
    txt = `${txt}<p class="random-text">${text[i]}</p>`
  }
